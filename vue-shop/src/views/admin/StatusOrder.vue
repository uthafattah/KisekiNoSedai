<template>
	<v-data-table item-key="name" class="elevation-1" :loading="loading" loading-text="Loading... Please wait" :headers="headers" :options.sync="options" :server-items-length="status_orders.total" :items="status_orders.data" :footer-props="footerProps">
		<template v-slot:top>
						...
						<v-form ref="form" v-model="valid" v-on:submit.stop.prevent="save">
							<v-card-text>
								<v-container>
									<v-row>
										<v-col cols="12" sm="12">
											<v-text-field v-model="editedItem.name" :rules="[rules.required, rules.min]" label="Status Order Name" v-if="editedIndex > -1" />
											<v-text-field v-model="editedItem.name" :success-messages="msg.success" :error-messages="msg.error" :rules="[rules.required, rules.min]" :blur="checkStatus" label="Status Order Name" v-else/>
										</v-col>
									</v-row>
								</v-container>
							</v-card-text>
	...
	export default {
		data: () => ({
			...
			footerProps: {
				...
				itemsPerPageText: 'Status Order Per Page',
		...
		methods: {
			...
			verifyStatus() {
				if (this.editedItem.name.length >= 5) {
					...
					.catch(err => {
						...
						this.msg.success = "";
						this.msg.error = "Status Order Already Exists";
					});
				} else {
					this.msg.success = "";
					this.msg.error = "";
			...
			searchIt(e) {
				if(e) {
					...
					.then(res => this.status_orders = res.data.status_orders)
					.catch(err => console.dir(err.response))
				}
			},
			initialize () {
